...
xfce_notify_log_initable_real_init(GInitable *initable, GCancellable *cancellable, GError **error) {
    ...
    if (G_LIKELY(success)) {
        ...
        if (G_UNLIKELY(rc != SQLITE_OK)) {
            ...
            sqlite3_db_config(log->db, SQLITE_DBCONFIG_DEFENSIVE, 1);
...
xfce_notify_log_read(XfceNotifyLog *log, const gchar *start_after_id, guint count) {
    ...
    if (G_LIKELY(rc == SQLITE_OK)) {
        rc = sqlite3_bind_int(stmt,
                              BIND_INDEX(log->stmt_read, BIND_LIMIT),
                              count);
    }
...
xfce_notify_log_real_write(XfceNotifyLog *log, XfceNotifyLogEntry *entry) {
    ...
    DBG("new entry: id=%s, ts=%" G_GINT64_FORMAT ", tsid=%s, aid=%s, an=%s, iid=%s, sum=%s, bod=%s, act=%p, et=%d, ir=%d",
        entry->id,
        timestamp,
        g_time_zone_get_identifier(g_date_time_get_timezone(entry->timestamp)),
        entry->app_id,
        entry->app_name,
        entry->icon_id,
        entry->summary,
...
xfce_notify_log_entry_ref(XfceNotifyLogEntry *entry) {
    ...
    g_return_val_if_fail(g_atomic_int_get(&entry->ref_count) > 0, NULL);
