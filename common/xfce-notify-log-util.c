...
notify_pixbuf_from_image_data(GVariant *image_data) {
    ...
    correct_len = (height - 1) * rowstride + width * ((channels * bits_per_sample + 7) / 8);
...
static gchar *
notify_read_from_desktop_file(const gchar *desktop_file_path,
                              const gchar *key) {
...
static gchar *
notify_get_from_desktop_file_resolved(const gchar *desktop_file,
                                      const gchar *key) {
...
xfce_notify_log_get_icon_folder(void) {
    ...
    if(G_UNLIKELY(folder == NULL)) {
        folder = g_strconcat(g_get_user_cache_dir(), G_DIR_SEPARATOR_S, "xfce4", G_DIR_SEPARATOR_S, "notifyd", G_DIR_SEPARATOR_S, "icons", NULL);
    }
...
static void
xfce_notify_clear_icon_cache(void) {
...
static void
xfce_notify_clear_log_dialog_cb(GtkWidget *dialog,
                                gint response,
...
xfce_notify_clear_log_dialog(XfceNotifyLogGBus *log,
    ...
    g_signal_connect_data(dialog, "response", G_CALLBACK(xfce_notify_clear_log_dialog_cb), rdata, notify_closure_free, 0);
