...
notification_plugin_configure(XfcePanelPlugin *plugin,
    ...
    g_signal_connect(G_OBJECT(dialog), "response", G_CALLBACK(notification_plugin_configure_response), notification_plugin);
    ...
    xfconf_g_property_bind(notification_plugin->channel, SETTING_LOG_ICON_SIZE, G_TYPE_INT, G_OBJECT(gtk_builder_get_object(builder, "log_icon_size")), "value");

    log_display_limit = xfconf_channel_get_int(notification_plugin->channel,
                                               SETTING_LOG_DISPLAY_LIMIT,
                                               DEFAULT_LOG_DISPLAY_LIMIT);
    gtk_adjustment_set_value(GTK_ADJUSTMENT(gtk_builder_get_object(builder, "log_display_limit_adj")), log_display_limit);
    xfconf_g_property_bind(notification_plugin->channel, SETTING_LOG_DISPLAY_LIMIT, G_TYPE_INT, G_OBJECT(gtk_builder_get_object(builder, "log_display_limit")), "value");

    xfconf_g_property_bind(notification_plugin->channel, SETTING_LOG_ONLY_TODAY, G_TYPE_BOOLEAN, G_OBJECT(gtk_builder_get_object(builder, "show_only_today")), "active");

    xfconf_g_property_bind(notification_plugin->channel, SETTING_HIDE_CLEAR_PROMPT, G_TYPE_BOOLEAN, G_OBJECT(gtk_builder_get_object(builder, "hide_clear_prompt")), "active");

    xfconf_g_property_bind(notification_plugin->channel, SETTING_HIDE_ON_READ, G_TYPE_BOOLEAN, G_OBJECT(gtk_builder_get_object(builder, "hide_on_read")), "active");

    xfconf_g_property_bind(notification_plugin->channel, SETTING_SHOW_IN_MENU, G_TYPE_STRING, G_OBJECT(gtk_builder_get_object(builder, "show_in_menu")), "active-id");

    xfconf_g_property_bind(notification_plugin->channel, SETTING_AFTER_MENU_SHOWN, G_TYPE_STRING, G_OBJECT(gtk_builder_get_object(builder, "after_menu_shown")), "active-id");
...
notification_plugin_about(XfcePanelPlugin *plugin) {
    ...
    gtk_show_about_dialog(NULL,
                          ...
                          PACKAGE_NAME,
                          "comments",
                          _("This is the notification plugin"),
                          "website",
                          PLUGIN_WEBSITE,
                          "copyright",
                          _("Copyright \xc2\xa9 2017 Simon Steinbeiß\n"),
                          "authors",
                          auth,
                          NULL);
}
