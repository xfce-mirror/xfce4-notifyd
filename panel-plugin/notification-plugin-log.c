...
notification_plugin_clear_log_dialog (GtkWidget *widget, gpointer user_data)
{
  ...
  if (xfconf_channel_get_bool (notification_plugin->channel, SETTING_HIDE_CLEAR_PROMPT, FALSE)) {
    xfce_notify_org_xfce_notifyd_log_call_clear(notification_plugin->log, NULL, NULL, NULL);
  } else {
...
notification_plugin_mark_all_read(GtkWidget *widget, NotificationPlugin *notification_plugin) {
  xfce_notify_org_xfce_notifyd_log_call_mark_all_read(notification_plugin->log, NULL, NULL, NULL);
}
...
notification_plugin_menu_populate (NotificationPlugin *notification_plugin)
{
  ...
  if (notification_plugin->log != NULL) {
    ...
    if (!xfce_notify_org_xfce_notifyd_log_call_list_sync(notification_plugin->log,
                                                         "",
                                                         log_display_limit,
                                                         only_unread,
                                                         &entriesv,
...
G_GNUC_END_IGNORE_DEPRECATIONS
    ...
    if (mark_all_read) {
      xfce_notify_org_xfce_notifyd_log_call_mark_all_read(notification_plugin->log, NULL, NULL, NULL);
        ...
        xfce_notify_org_xfce_notifyd_log_call_mark_read(notification_plugin->log, (const gchar *const *)ids, NULL, NULL, NULL);
