...
static void
cb_menu_selection_done(GtkMenuShell *menu,
                       NotificationPlugin *notification_plugin);
...
notification_plugin_popup_menu(NotificationPlugin *notification_plugin) {
    ...
    g_signal_connect(menu, "selection-done", G_CALLBACK(cb_menu_selection_done), notification_plugin);
...
notification_plugin_bus_proxy_connected(GObject *source,
    ...
    if(G_UNLIKELY(notification_plugin->log == NULL)) {
        ...
        g_signal_connect_swapped(notification_plugin->log, "row-added", G_CALLBACK(notification_plugin_log_changed), notification_plugin);
        g_signal_connect_swapped(notification_plugin->log, "row-changed", G_CALLBACK(notification_plugin_log_changed), notification_plugin);
        g_signal_connect_swapped(notification_plugin->log, "row-deleted", G_CALLBACK(notification_plugin_log_changed), notification_plugin);
        g_signal_connect_swapped(notification_plugin->log, "truncated", G_CALLBACK(notification_plugin_log_changed), notification_plugin);
        g_signal_connect_swapped(notification_plugin->log, "cleared", G_CALLBACK(notification_plugin_log_changed), notification_plugin);
        xfce_notify_log_gbus_call_has_unread(notification_plugin->log,
                                             NULL,
                                             (GAsyncReadyCallback) notification_plugin_has_unread_ready,
                                             notification_plugin);
...
notification_plugin_new(XfcePanelPlugin *panel_plugin) {
    ...
    g_signal_connect(notification_plugin->channel, "property-changed::" SETTING_HIDE_ON_READ, G_CALLBACK(cb_hide_on_read_changed), notification_plugin);
    ...
    g_signal_connect(notification_plugin->button, "button-press-event", G_CALLBACK(cb_button_pressed), notification_plugin);
    g_signal_connect_swapped(gtk_icon_theme_get_default(), "changed", G_CALLBACK(notification_plugin_update_icon), notification_plugin);

    /* Start monitoring the "do not disturb" setting in xfconf */
    g_signal_connect(G_OBJECT(notification_plugin->channel), "property-changed::"
                                                             "/do-not-disturb",
                     G_CALLBACK(notification_plugin_dnd_updated),
                     notification_plugin);
...
notification_plugin_construct(XfcePanelPlugin *plugin) {
    ...
    g_signal_connect(G_OBJECT(plugin), "free-data", G_CALLBACK(notification_plugin_free), notification_plugin);

    g_signal_connect(G_OBJECT(plugin), "size-changed", G_CALLBACK(notification_plugin_size_changed), notification_plugin);

    /* show the configure menu item and connect signal */
    xfce_panel_plugin_menu_show_configure(plugin);
    g_signal_connect(G_OBJECT(plugin), "configure-plugin", G_CALLBACK(notification_plugin_configure), notification_plugin);

    /* show the about menu item and connect signal */
    xfce_panel_plugin_menu_show_about(plugin);
    g_signal_connect(G_OBJECT(plugin), "about", G_CALLBACK(notification_plugin_about), NULL);
}
