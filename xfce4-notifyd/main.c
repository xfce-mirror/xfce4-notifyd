...
main(int argc,
    ...
    if(!xndaemon) {
        xfce_message_dialog(NULL, _("Xfce Notify Daemon"), "dialog-error", _("Unable to start notification daemon"), error->message, "application-exit", GTK_RESPONSE_ACCEPT, NULL);
        g_error_free(error);
        return 1;
    }
