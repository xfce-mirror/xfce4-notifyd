...
static void
xfce_notify_log_viewer_set_property(GObject *obj,
                                    guint prop_id,
                                    const GValue *value,
                                    GParamSpec *pspec);
static void
xfce_notify_log_viewer_get_property(GObject *obj,
                                    guint prop_id,
                                    GValue *value,
                                    GParamSpec *pspec);
static void
...
xfce_notify_log_viewer_listbox_display_header_func(GtkListBoxRow *row,
                                                   ...
                                                   gpointer user_data);
static gboolean
xfce_notify_log_viewer_listbox_button_press(GtkWidget *listbox,
                                            GdkEventButton *evt,
                                            XfceNotifyLogViewer *viewer);
static void
xfce_notify_log_viewer_selection_changed(GtkWidget *listbox,
                                         XfceNotifyLogViewer *viewer);

static void
xfce_notify_log_viewer_scroll_edge_reached(XfceNotifyLogViewer *viewer,
                                           GtkPositionType pos);

static void
xfce_notify_log_viewer_log_row_added(XfceNotifyLogViewer *viewer,
                                     const gchar *entry_id);
static void
xfce_notify_log_viewer_log_row_changed(XfceNotifyLogViewer *viewer,
                                       const gchar *entry_id);
static void
xfce_notify_log_viewer_log_row_deleted(XfceNotifyLogViewer *viewer,
                                       const gchar *entry_id);
static void
xfce_notify_log_viewer_log_truncated(XfceNotifyLogViewer *viewer,
                                     guint n_kept_entries);
static void
xfce_notify_log_viewer_log_cleared(XfceNotifyLogViewer *viewer);


G_DEFINE_TYPE(XfceNotifyLogViewer,
...
xfce_notify_log_viewer_constructed(GObject *obj) {
    ...
    g_signal_connect_swapped(viewer->scroller, "edge-reached", G_CALLBACK(xfce_notify_log_viewer_scroll_edge_reached), viewer);
    ...
    g_signal_connect(viewer->listbox, "button-press-event", G_CALLBACK(xfce_notify_log_viewer_listbox_button_press), viewer);
    g_signal_connect(viewer->listbox, "selected-rows-changed", G_CALLBACK(xfce_notify_log_viewer_selection_changed), viewer);
    ...
    g_signal_connect_swapped(viewer->channel, "property-changed::" DATETIME_FORMAT_PROP, G_CALLBACK(xfce_notify_log_viewer_populate), viewer);
    g_signal_connect_swapped(viewer->channel, "property-changed::" DATETIME_CUSTOM_FORMAT_PROP, G_CALLBACK(xfce_notify_log_viewer_populate), viewer);

    if(viewer->log != NULL) {
        g_signal_connect_swapped(viewer->log, "row-added", G_CALLBACK(xfce_notify_log_viewer_log_row_added), viewer);
        g_signal_connect_swapped(viewer->log, "row-changed", G_CALLBACK(xfce_notify_log_viewer_log_row_changed), viewer);
        g_signal_connect_swapped(viewer->log, "row-deleted", G_CALLBACK(xfce_notify_log_viewer_log_row_deleted), viewer);
        g_signal_connect_swapped(viewer->log, "truncated", G_CALLBACK(xfce_notify_log_viewer_log_truncated), viewer);
        g_signal_connect_swapped(viewer->log, "cleared", G_CALLBACK(xfce_notify_log_viewer_log_cleared), viewer);
    }
...
notify_icon_for_entry(XfceNotifyLogEntry *entry,
...
static void
log_entry_mark_read_clicked(GtkWidget *mi,
                            XfceNotifyLogViewer *viewer) {
...
static void
set_last_clicked_row(XfceNotifyLogViewer *viewer,
                     GtkWidget *row) {
    ...
    if(evt->type == GDK_BUTTON_PRESS) {
        if(evt->button == GDK_BUTTON_PRIMARY) {
            ...
            if(n_selected > 0) {
                ...
                g_signal_connect(mi, "activate", G_CALLBACK(log_entry_mark_read_clicked), viewer);
                g_free(label);
                ...
                G_GNUC_END_IGNORE_DEPRECATIONS
                gtk_menu_shell_append(GTK_MENU_SHELL(menu), mi);
                g_signal_connect_swapped(mi, "activate", G_CALLBACK(log_entry_delete_clicked), viewer);
                g_free(label);
...
xfce_notify_log_viewer_insert_entry(XfceNotifyLogViewer *viewer,
    ...
    g_signal_connect(eventbox, "button-press-event", G_CALLBACK(xfce_notify_log_viewer_listbox_row_button_press), viewer);
...
static void
xfce_notify_log_viewer_append_entries(XfceNotifyLogViewer *viewer,
                                      GList *entries) {
    GDateTime *today;
...
static void
has_unread_finished(GObject *source,
                    GAsyncResult *res,
                    XfceNotifyLogViewer *viewer) {
    GError *error = NULL;
...
static void
added_row_fetched(GObject *source,
                  GAsyncResult *res,
                  XfceNotifyLogViewer *viewer) {
...
static void
changed_row_fetched(GSource *source,
                    GAsyncResult *res,
                    RowChangeData *data) {
    GVariant *entryv = NULL;
    GError *error = NULL;
