...
known_application_log_counts_fetched(GObject *source, GAsyncResult *res, SettingsPanel *panel) {
    ...
    if (source != NULL
        && res != NULL
        && xfce_notify_org_xfce_notifyd_log_call_get_app_id_counts_finish(XFCE_NOTIFY_ORG_XFCE_NOTIFYD_LOG(source),
                                                                          &app_id_countsv,
                                                                          res,
                                                                          &error))
    {
...
xfce4_notifyd_known_applications_changed(SettingsPanel *panel) {
    if (panel->log != NULL) {
        xfce_notify_org_xfce_notifyd_log_call_get_app_id_counts(panel->log,
                                                                NULL,
     ...
     char **argv)
{
    ...
    panel->log = xfce_notify_org_xfce_notifyd_log_proxy_new_for_bus_sync(G_BUS_TYPE_SESSION,
                                                                         0,
                                                                         "org.xfce.Notifyd",
                                                                         "/org/xfce/Notifyd",
                                                                         NULL,
